(function($){
	$( document ).ready( function() {
		...
		$iframe_preview = 'old' === sevenp_blu_customizer_data.is_old_wp ? $( '#customize-preview' ) : $( '.wp-full-overlay' ),
		all_device_classes = 'old' === sevenp_blu_customizer_data.is_old_wp ? 'sevenp_blu_phone sevenp_blu_tablet' : 'preview-tablet preview-mobile preview-desktop',
		tablet_class = 'old' === sevenp_blu_customizer_data.is_old_wp ? 'sevenp_blu_tablet' : 'preview-tablet';
		phone_class = 'old' === sevenp_blu_customizer_data.is_old_wp ? 'sevenp_blu_phone' : 'preview-mobile';
		desktop_class = 'old' === sevenp_blu_customizer_data.is_old_wp ? '' : 'preview-desktop';
		...
		$('select[data-customize-setting-link="sevenp_blu[footer_columns]"]').et_select_image({ apply_value_to: 'body' });

		$( '.sevenp_blu_reset_slider' ).click( function () {
		...
		$( '#accordion-section-sevenp_blu_mobile_tablet h3, #accordion-panel-sevenp_blu_mobile h3' ).click( function () {
			$iframe_preview.removeClass( all_device_classes ).addClass( tablet_class );

			if ( 'old' !== sevenp_blu_customizer_data.is_old_wp ) {
				$( '#customize-footer-actions .devices' ).css( { display: 'none' } );
			}
		});

		$( '#accordion-section-sevenp_blu_mobile_phone h3, #accordion-section-sevenp_blu_mobile_menu h3' ).click( function () {
		...
		var $vertical_nav_option                  = $( '#customize-control-sevenp_blu-vertical_nav' )
			$vertical_nav_input                   = $vertical_nav_option.find( 'input[type=checkbox]' ),
			$nav_fullwidth_control                = $( '#customize-control-sevenp_blu-nav_fullwidth' ),
			$hide_navigation_until_scroll_control = $('#customize-control-sevenp_blu-hide_nav'),
			$header_style_option                  = $( '#customize-control-sevenp_blu-header_style select' ),
			$secondary_menu_options               = $( '#accordion-section-sevenp_blu_header_secondary' ),
			$slide_header_section                 = $( '#accordion-section-sevenp_blu_header_slide' ),
			$show_top_bar_input                   = $( '#customize-control-sevenp_blu-slide_nav_show_top_bar input[type=checkbox]' ),
			$top_bar_related_options              = $( '#customize-control-sevenp_blu-slide_nav_bg_top, #customize-control-sevenp_blu-slide_nav_top_color, #customize-control-sevenp_blu-slide_nav_search, #customize-control-sevenp_blu-slide_nav_search_bg'),
			$primary_header_options               = $( '#customize-control-sevenp_blu-primary_nav_font_size, #customize-control-sevenp_blu-primary_nav_font_spacing, #customize-control-sevenp_blu-primary_nav_font, #customize-control-sevenp_blu-primary_nav_font_style, #customize-control-sevenp_blu-menu_link_active, #customize-control-sevenp_blu-primary_nav_dropdown_bg, #customize-control-sevenp_blu-primary_nav_dropdown_line_color, #customize-control-sevenp_blu-primary_nav_dropdown_link_color, #customize-control-sevenp_blu-primary_nav_dropdown_animation, #customize-control-sevenp_blu-fixed_primary_nav_font_size, #customize-control-sevenp_blu-fixed_secondary_nav_bg, #customize-control-sevenp_blu-fixed_menu_link, #customize-control-sevenp_blu-fixed_secondary_menu_link, #customize-control-sevenp_blu-fixed_menu_link_active' ),
			$slide_only_options                   = $( '#customize-control-sevenp_blu-slide_nav_width, #customize-control-sevenp_blu-slide_nav_search, #customize-control-sevenp_blu-slide_nav_search_bg, #customize-control-sevenp_blu-slide_nav_font_size, #customize-control-sevenp_blu-slide_nav_top_font_size' ),
			$fullscreen_only_options              = $( '#customize-control-sevenp_blu-fullscreen_nav_font_size, #customize-control-sevenp_blu-fullscreen_nav_top_font_size' ),
			$vertical_orientation                 = $( '#customize-control-sevenp_blu-vertical_nav_orientation' ),
			$menu_height                          = $( '#customize-control-sevenp_blu-menu_height' ),
			$menu_margin_top                        = $( '#customize-control-sevenp_blu-menu_margin_top' );
		...
		$('#customize-theme-controls').on( 'change', '#customize-control-sevenp_blu-vertical_nav input[type=checkbox]', function(){
		...
		$( '#customize-theme-controls' ).on( 'change', '#customize-control-sevenp_blu-header_style select', function(){
			$input = $(this);
		...
		$( '#customize-theme-controls' ).on( 'change', '#customize-control-sevenp_blu-slide_nav_show_top_bar input[type=checkbox]', function(){
			$input = $(this);
		...
		function toggle_sidebar_width_control() {
			$checkbox              = $('#customize-control-sevenp_blu-use_sidebar_width input[type="checkbox"]'),
			$sidebar_width_control = $( '#customize-control-sevenp_blu-sidebar_width' );
		...
		$('#customize-theme-controls').on( 'change', '#customize-control-sevenp_blu-use_sidebar_width input[type=checkbox]', function(){
			toggle_sidebar_width_control();
		});
	...
	api.ET_ColorAlphaControl = api.Control.extend({
		ready: function() {
			...
			*/
			if ( 'sevenp_blu[footer_menu_text_color]' === this.id ) {
				...
				this.setting.bind( 'change', function( value ){
					...
					api( 'sevenp_blu[footer_menu_active_link_color]' ).set( value );
					...
					api.control( 'sevenp_blu[footer_menu_active_link_color]' ).container.find( '.color-picker-hex' )
	...
	$( window ).load( function() {
		var $et_custom_footer_credits_disable_control = $('#customize-control-sevenp_blu-disable_custom_footer_credits input'),
			$et_custom_footer_credits_control         = $('#customize-control-sevenp_blu-custom_footer_credits');
		...
		if ( $( '#accordion-section-sevenp_blu_buttons' ).length ) {
			var $icon_options_trigger = $( '#customize-control-sevenp_blu-all_buttons_icon select' ),
		...
		function trigger_button_options( trigger_val ) {
			...
			$.each( icon_options_set, function( i, option_name ) {
				if ( 'yes' === trigger_val ) {
					$( '#customize-control-sevenp_blu-' + option_name ).show();
				} else {
					$( '#customize-control-sevenp_blu-' + option_name ).hide();
				}
			} );
		}
